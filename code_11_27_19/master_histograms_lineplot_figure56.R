...
lineplot_SP=ggplot(species,aes(x=date))+geom_line(aes(y=chla,group=product,color=product),size=.3)+geom_point(aes(y=chla,group=product,color=product),size=.4)+
  ...
  facet_grid(~year, scales="free")+labs(x="Month")+labs(y="MODIS minus VIIRS")+theme(legend.position=c(1,.95),legend.justification = c(.9,.9))+
  theme(axis.text = element_text(size=13),axis.title = element_text(size=14),legend.text=element_text(size=10),legend.title = element_text(size=10),strip.text.y = element_text(size = 10),strip.text.x = element_text(size = 14), strip.background = element_blank(),axis.text.x = element_text(angle=45,vjust=.7))+
  theme(legend.key.size = unit(.9,'lines'))+ylim(c(-.07,.1))+
  geom_text(data=ann_text,aes(x=date,y=chla),label="B",size=4.5)+
  ...
  geom_text(data=ann_text_2017,aes(x=date,y=chla),label="Box 1",size=4.5)+
  geom_text(data=ann_text_2018,aes(x=date,y=chla),label="Box 2",size=4.1)+theme(panel.background = element_blank(),legend.key=element_blank())+
...
lineplot_chla_eco=ggplot(chla_eco,aes(x=date))+geom_line(aes(y=chla,group=product,color=product),size=.3)+geom_point(aes(y=chla,group=product,color=product),size=.4)+
  ...
  facet_grid(~year, scales="free")+labs(x="Month")+labs(y="MODIS minus VIIRS")+theme(legend.position=c(1,1),legend.justification = c(.9,.9))+
  theme(axis.text = element_text(size=13),axis.title = element_text(size=14),legend.text=element_text(size=10),legend.title = element_text(size=10),strip.text.y = element_text(size = 10),strip.text.x = element_text(size = 14), strip.background = element_blank(),axis.text.x = element_text(angle=45,vjust=.7))+
  theme(legend.key.size = unit(.9,'lines'))+ylim(c(-.07,.1))+
  geom_text(data=ann_text,aes(x=date,y=chla),label="A",size=4.5)+theme(panel.background = element_blank(),legend.key=element_blank())+
  theme(axis.line.x = element_line(color="black", size = 1),
        axis.line.y = element_line(color="black", size = 1))+
  theme(plot.margin=margin(t = 0, r = 15, b = 0, l = 0, unit = "pt"))
