...
histtt_chla=ggplot(chla,aes(x=chla,group=product))+geom_density(aes(fill=product),alpha=.4)+
  ...
  theme(legend.key.size = unit(.5,'lines'))+xlim(c(-.07,.13))+ylim(c(0,100))+labs(x="MODIS minus other products - Chlorophyll")+labs(y="Density")+
  theme(legend.position=c(.5,.85))+annotate(geom = "text",x=-.07,y=100,label="B",size=4)+
...
histtt_eco=ggplot(eco,aes(x=chla,group=product))+geom_density(aes(fill=product),alpha=.4)+
  ...
  theme(legend.key.size = unit(.5,'lines'))+xlim(c(-.07,.13))+ylim(c(0,100))+labs(x="MODIS minus other products - EcoCast")+labs(y="Density")+
  theme(legend.position=c(.55,.9))+annotate(geom = "text",x=-.07,y=100,label="A",size=4)+
  theme(axis.text = element_text(size=10),axis.title = element_text(size=10),legend.text=element_text(size=10),legend.title = element_text(size=10),strip.text.y = element_text(size = 6),strip.text.x = element_text(size = 6), strip.background = element_blank())

histtt_eco

datatype="histogram_line_8Day"
...
master=rbind(chla %>% mutate(master_p="chla"),eco%>% mutate(master_p="eco"))
mean_diff=master %>% mutate(chl2=chla)%>% group_by(product,master_p) %>% summarise(mean(chl2,na.rm=T)) %>% arrange(master_p)
mean_diff_abs=master %>% mutate(chl2=abs(chla))%>% group_by(product,master_p) %>% summarise(mean(chl2,na.rm=T))%>% arrange(master_p)
