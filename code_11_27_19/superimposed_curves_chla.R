...
modis_2018_late=unique (grep(paste(to_matchL,collapse="|"),modis, value=TRUE))%>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) %>% mean(.,na.rm=T)
...
viirs_2018_late=unique (grep(paste(to_matchL,collapse="|"),viirs, value=TRUE)) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) %>% mean(.na.rm=T)
...
f=f+ylab("Scaled chlorophyll influence & density distribution")+xlab(bquote("log chlorophyll (mg "*~m^-3*")"))+theme(legend.position=c(.18,.9),legend.justification = c(.9,.9))+
  geom_text(aes(x=-4,y=1),label="A",size=5)+theme(panel.background = element_blank(),legend.key=element_blank())+
  theme(axis.line.x = element_line(color="black", size = 1),
        axis.line.y = element_line(color="black", size = 1))+
  theme(plot.margin=margin(t = 10, r = 0, b = 0, l = 10, unit = "pt"))
...
f=f+geom_vline(xintercept=2,linetype="dashed")+geom_vline(xintercept=0,linetype="dashed")
f=f+ylab("Scaled chlorophyll influence & density distribution")+xlab(bquote("log chlorophyll (mg "*~m^-3*")"))+theme(legend.position=c(.18,.9),legend.justification = c(.9,.9))+
  geom_text(aes(x=-3,y=1),label="B",size=5)+theme(panel.background = element_blank(),legend.key=element_blank())+
  theme(axis.line.x = element_line(color="black", size = 1),
        axis.line.y = element_line(color="black", size = 1))+
  theme(plot.margin=margin(t = 10, r = 0, b = 0, l = 10, unit = "pt"))
f=f+scale_x_continuous(labels=mult_format())
...
f=f+ylab("Scaled chlorophyll influence & density distribution")+xlab(bquote("log chlorophyll (mg "*~m^-3*")"))+theme(legend.position=c(.9,.25),legend.justification = c(.9,.9))+
  geom_text(aes(x=3,y=1),label="C",size=5)+theme(panel.background = element_blank(),legend.key=element_blank())+
  theme(axis.line.x = element_line(color="black", size = 1),
        axis.line.y = element_line(color="black", size = 1))+
  theme(plot.margin=margin(t = 10, r = 0, b = 0, l = 10, unit = "pt"))
f=f+scale_x_continuous(labels=mult_format())
