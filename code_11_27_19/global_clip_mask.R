...
plot_global=function(raster,datatype,plus,minus,outputDir){
  ...
  image.plot(raster, col=pal(ncolors), breaks=breaks, ylab="", xlab="", xlim=c(-180, 180),ylim=c(-90.00001, 90),legend.cex=.7)
...
rs=list(modis,viirs,glo_avw,glo_gsm,esa)
datatypes=c("MODIS","VIIRS","GlobColour AVW","GlobColour GSM","OC-CCI")
# a=mcmapply(function(x,y)names(x)=y,rs,datatypes)
...
mcmapply(plot_global,raster=rs,datatype=datatypes,MoreArgs=list(plus=plus,minus=minus,outputDir = outputDir),mc.cores =8)
