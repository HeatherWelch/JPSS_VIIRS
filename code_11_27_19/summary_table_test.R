...
mean_diff_chla=master %>% mutate(chl2=chla)%>% group_by(product) %>% summarise(mean(chl2,na.rm=T))
mean_diff_abs_chla=master %>% mutate(chl2=abs(chla))%>% group_by(product) %>% summarise(mean(chl2,na.rm=T))
...
write.csv(master1,paste(outputDir,"summaryTable_test.csv"))
