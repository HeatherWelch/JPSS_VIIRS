...
run_ecocast=function(data_product,date,datapath){
  ...
  ## paths for JPSS ####
  envdir2=paste(datapath,"/",data_product,"/Satellite_mask_resub_cara/",sep="") 
  outdir=paste(datapath,"/",data_product,"/EcoCastRuns_resub_cara/",sep="") 
  if(!dir.exists(outdir)){dir.create(outdir)}
  ecocastdir=paste(outdir,"output/",sep="")
  if(!dir.exists(ecocastdir)){dir.create(ecocastdir)}
  outdir_M=paste(ecocastdir,"mean/",sep="") 
...
# lapply(dates,FUN=run_ecocast,data_product=data_product,datapath=datapath)
# 
#### viirs NASA 2015 - 2018; 8 day (running w new sla) ####
dates=list.files("/Users/heatherwelch/Dropbox/JPSS/viirsNASA_8Day/Satellite_mask_resub_cara")
data_product="viirsNASA_8Day"
datapath="/Users/heatherwelch/Dropbox/JPSS"
