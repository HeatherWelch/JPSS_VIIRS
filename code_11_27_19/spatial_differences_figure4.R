...
PlotPNGs<-function(stack,datatype,outputDir){
  ...
  ####### produce png ####
  png(paste(outputDir,datatype,".png",sep=''),width=15,height=15,units='cm',res=400)
...
PlotPNGs_difference<-function(stack,product,outputDir,H,L,countour_ras){
  ...
  ####### produce png ####
  png(paste(outputDir,product,"_M-V_difference2.png",sep=''),width=15,height=15,units='cm',res=400)
  par(mar=c(3,3,.5,.5),las=1,font=2)
  par(mfrow=c(1,1))
  par(oma=c(0,0,0,1))
...
PlotPNGs_difference(stack = ecocast,product = "EcoCast's fishing suitability",outputDir = outputDir,H=H,L=L,countour_ras = countour_ras)
