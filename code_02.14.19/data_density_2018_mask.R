...
modis_2018_early=unique (grep(paste(to_matchE,collapse="|"),modis_2018, value=TRUE)) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 
modis_2018_early[!is.na(modis_2018_early)]=1
modis_2018_early=modis_2018_early %>% calc(.,fun=sum,na.rm=T)%>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 

modis_2018_late=unique (grep(paste(to_matchL,collapse="|"),modis_2018, value=TRUE))%>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 
modis_2018_late[!is.na(modis_2018_late)]=1
modis_2018_late=modis_2018_late %>% calc(.,fun=sum,na.rm=T)%>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 

viirs_2018=list.files(viirsDir,full.names = T,recursive = T,pattern = ".grd")%>% grep("2016-10-11",.,invert=T,value=T)%>% grep("2018",.,,value=T)
viirs_2018=unique (grep(paste(to_match_date,collapse="|"),viirs_2018, value=TRUE)) %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 

viirs_2018_early=unique (grep(paste(to_matchE,collapse="|"),viirs_2018, value=TRUE)) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 
viirs_2018_early[!is.na(viirs_2018_early)]=1
viirs_2018_early=viirs_2018_early %>% calc(.,fun=sum,na.rm=T)%>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 

viirs_2018_late=unique (grep(paste(to_matchL,collapse="|"),viirs_2018, value=TRUE)) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 
viirs_2018_late[!is.na(viirs_2018_late)]=1
viirs_2018_late=viirs_2018_late %>% calc(.,fun=sum,na.rm=T)%>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 
...
datatype="data_density_2018_mask"
PlotPNGs_difference<-function(stack,datatype,outputDir){
  ...
  image.plot(stack[[1]],col=col,xlim=c(-130,-115),ylim=c(30,47),zlim=zlimitsE)
...
modis_2018_early=unique (grep(paste(to_matchE,collapse="|"),modis_2018, value=TRUE)) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 
#modis_2018_early[!is.na(modis_2018_early)]=1
modis_2018_early=modis_2018_early %>% calc(.,fun=mean,na.rm=F)%>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 

modis_2018_late=unique (grep(paste(to_matchL,collapse="|"),modis_2018, value=TRUE))%>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 
#modis_2018_late[!is.na(modis_2018_late)]=1
modis_2018_late=modis_2018_late %>% calc(.,fun=mean,na.rm=F)%>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 
...
viirs_2018_early=unique (grep(paste(to_matchE,collapse="|"),viirs_2018, value=TRUE)) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 
#viirs_2018_early[!is.na(viirs_2018_early)]=1
viirs_2018_early=viirs_2018_early %>% calc(.,fun=mean,na.rm=F)%>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 

viirs_2018_late=unique (grep(paste(to_matchL,collapse="|"),viirs_2018, value=TRUE)) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 
#viirs_2018_late[!is.na(viirs_2018_late)]=1
viirs_2018_late=viirs_2018_late %>% calc(.,fun=mean,na.rm=F)%>%mask(.,studyarea) %>% crop(.,extent(studyarea)) 


master=stack(modis_2018_early,viirs_2018_early,modis_2018_late,viirs_2018_late)
outputDir="/Users/heatherwelch/Dropbox/JPSS/plots_02.14.19/"
datatype="data_density_2_2018_mask"
PlotPNGs_difference<-function(stack,datatype,outputDir){
