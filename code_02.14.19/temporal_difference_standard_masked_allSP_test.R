...
blshtr1=blshtrk%>%  mutate(product="Blueshark-Track")

master=do.call("rbind",list(lbst1,blshObs1,casl1,swor1,blshtr1))  #%>% .[complete.cases(.),]
...
lineplot=ggplot(master,aes(x=date))+geom_line(aes(y=chla,group=product,color=product),size=.3)+geom_point(aes(y=chla,group=product,color=product),size=.4)+
  ...
  facet_grid(~year, scales="free")+labs(x="Date")+labs(y="Chlorophyl difference ")+theme(legend.position=c(.4,.2),legend.justification = c(.9,.9))+
...
datatype="standard_difference_mask_allSP_test"
