...
modis_2018_early=unique (grep(paste(to_matchE,collapse="|"),modis_2018, value=TRUE)) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) %>% calc(.,fun=mean,na.rm=T)

modis_2018_late=unique (grep(paste(to_matchL,collapse="|"),modis_2018, value=TRUE))%>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) %>% calc(.,fun=mean,na.rm=T)
...
viirs_2018_early=unique (grep(paste(to_matchE,collapse="|"),viirs_2018, value=TRUE)) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) %>% calc(.,fun=mean,na.rm=T)

viirs_2018_late=unique (grep(paste(to_matchL,collapse="|"),viirs_2018, value=TRUE)) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) %>% calc(.,fun=mean,na.rm=T)
