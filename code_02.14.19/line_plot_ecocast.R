...
dates_nochla=list.files(noCHLADir,full.names = T,recursive = T,pattern = "mean.grd")%>% grep("2016-10-11",.,invert=T,value=T)%>% grep("2016-09-07",.,invert=T,value=T) %>% gsub("/Users/heatherwelch/Dropbox/JPSS/viirs_8Day/EcoCastRuns/output/mean/EcoCast_-0.1_-0.1_-0.05_-0.9_0.9_","",.) %>% gsub("_mean.grd","",.)%>% gsub("_mean.grd","",.)
...
lineplot=ggplot(a,aes(x=date,y=chla))+geom_line(aes(group=sensor,color=sensor),size=.3)+geom_point(aes(group=sensor,color=sensor),size=.4)+
  ...
  facet_wrap(~year, scales="free_x", nrow=1)+labs(x="Date")+labs(y="Chlorophyl difference ")+theme(legend.position=c(.2,.9),legend.justification = c(.9,.9))+
