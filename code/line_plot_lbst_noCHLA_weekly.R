...
modisDir="/Users/heatherwelch/Dropbox/JPSS/modis_8DayBin/EcoCastRuns/lbst/predCIs"
viirsDir="/Users/heatherwelch/Dropbox/JPSS/viirs_8DayBin/EcoCastRuns/lbst/predCIs"
pmlEsaDir="/Users/heatherwelch/Dropbox/JPSS/pmlEsa_8Day/EcoCastRuns/lbst/predCIs"
noCHLADir="/Users/heatherwelch/Dropbox/JPSS/no_chlaBin/EcoCastRuns/lbst/predCIs"
...
dates_m=list.files(modisDir,full.names = T,recursive = T,pattern = "mean.grd")%>% grep("2016-10-11",.,invert=T,value=T)%>% grep("2016-09-07",.,invert=T,value=T) %>% gsub("/Users/heatherwelch/Dropbox/JPSS/modis_8DayBin/EcoCastRuns/lbst/predCIs/lbst_pa_","",.) %>% gsub("_mean.grd","",.)
names(sat_m)=dates_m
...
dates_v=list.files(viirsDir,full.names = T,recursive = T,pattern = "mean.grd")%>% grep("2016-09-07",.,invert=T,value=T) %>% gsub("/Users/heatherwelch/Dropbox/JPSS/viirs_8DayBin/EcoCastRuns/lbst/predCIs/lbst_pa_","",.) %>% gsub("_mean.grd","",.)
names(sat_v)=dates_v
...
dates_nochla=list.files(noCHLADir,full.names = T,recursive = T,pattern = "mean.grd")%>% grep("2016-10-11",.,invert=T,value=T)%>% grep("2016-09-07",.,invert=T,value=T) %>% gsub("/Users/heatherwelch/Dropbox/JPSS/no_chlaBin/EcoCastRuns/lbst/predCIs/lbst_pa_","",.) %>% gsub("_mean.grd","",.)
...
colnames(b)=c("chla","date")
b=b %>% mutate(year=as.factor(strtrim(as.character(date),4))) %>% filter(year!=2012&year!=2019)%>% mutate(month=as.factor(substr(as.character(date),6,7)))%>% filter(month!="01"&month!="07")
nochla_df=b %>% mutate(sensor="No Chl-a") #%>% left_join(a,.,by=c("full_TS"="date"))

master=do.call("rbind",list(m_df,v_df,pml_df,nochla_df))
...
lineplot=ggplot(master,aes(x=date,y=chla))+geom_line(aes(group=sensor,color=sensor),size=.3)+geom_point(aes(color=sensor),size=.6)+
  ...
  scale_color_manual("Sensor",values=c("Blended"="darkgoldenrod","MODIS"="coral1","VIIRS"="gray","No Chl-a"="black"))+
...
names(sat_nochla)=dates_nochla
nochla_stats=cellStats(sat_nochla,stat="mean")
nochla_stats_SD=cellStats(sat_nochla,stat="sd")
b=nochla_stats %>% as.data.frame() %>% mutate(date=as.Date(dates_nochla)) 
colnames(b)=c("chla","date")
b$upper=b$chla+nochla_stats_SD
b$lower=b$chla-nochla_stats_SD
b=b %>% mutate(year=as.factor(strtrim(as.character(date),4))) %>% filter(year!=2012&year!=2019)%>% mutate(month=as.factor(substr(as.character(date),6,7)))%>% filter(month!="01"&month!="07")
nochla_df=b %>% mutate(sensor="No Chl-a") #%>% left_join(a,.,by=c("full_TS"="date"))
...
lineplot=ggplot(master,aes(x=date,y=chla))+geom_line(aes(group=sensor,color=sensor),size=.3)+geom_point(aes(color=sensor),size=.4)+
  ...
  scale_x_date(date_breaks="month",date_labels = "%b",date_minor_breaks = "months")+
  scale_color_manual("Sensor",values=c("Blended"="cadetblue3","MODIS"="coral1","VIIRS"="gray","No Chl-a"="black"))+
  scale_fill_manual("",values=c("Blended"="cadetblue3","MODIS"="coral1","VIIRS"="gray","No Chl-a"="black"),guide=F)+
...
outputDir="/Users/heatherwelch/Dropbox/JPSS/plots/"
datatype="lbstaRibbon_nochla_weekly"
...
### haven't done anything here yet. Need to calculate difference. Might just drop blended
...
# nochla=list.files(noCHLADir,full.names = T,recursive = T,pattern = "mean.grd")%>% grep("2016-10-11",.,invert=T,value=T)%>% grep("2016-09-07",.,invert=T,value=T) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) %>% calc(.,mean,na.rm=T)
# 
# master=stack(modis,viirs,blend,nochla)
