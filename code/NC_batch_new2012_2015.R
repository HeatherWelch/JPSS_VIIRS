...
netcdf=list.files(netcdf_dir,pattern="*erdMBchla8day_LonPM180*",full.names = T)
template_native=raster(netcdf[1])
...
for(eco_year in years){
  ...
  var="l.blendChl"
  years=as.character(seq(from=as.Date("2015-02-01"),to=as.Date("2015-07-31"),by="day"))
  dir="/Volumes/SeaGate/ERD_DOM/EcoCast_CodeArchive/SpatialPredictions_EnvData/Satellite/"
  for(year in years){
    folder=paste0(dir,year)
    print(folder)
    a=raster(paste0(folder,"/",var,".grd"))
    b=resample(a,l.eke_mean.grd,method="bilinear")
    writeRaster(b,paste0(folder,"/",var),overwrite=T)
...
#### viirs 2015; 8 day ####
  netcdf_dir="/Users/Ecocast/Dropbox/JPSS/viirs_netcdf"
  output_dir="/Users/ecocast/Dropbox/JPSS/viirs_8Day";dir.create(output_dir)
  output_dir="/Users/ecocast/Dropbox/JPSS/viirs_8Day/Satellite";dir.create(output_dir)
  
  netcdf=list.files(netcdf_dir,pattern="*erdVHNchla8day*",full.names = T)
  template_native=raster(netcdf[1])
