...
Get_Env_Data_B_1=function(machine){
  ...
  if(machine=="lasdev"){
    ...
    source(paste0(path,"/Operationalizing_code_V3/2_load_libraries.R"),chdir = TRUE)
    source(paste0(path,"/Operationalizing_code_V3/4_predict_CIs.R"),chdir = TRUE)
    source(paste0(path,"/Operationalizing_code_V3/5_plot_EcoCast.R"),chdir = TRUE)
  }
...
for(get_date in SEQNCE){
  ...
  if(("analysed_sst.grd" %in% FileList_missing)==TRUE){ #only download if there is NOT a new jplG1SST layer
    sst=paste("http://oos.soest.hawaii.edu/erddap/griddap/NOAA_DHW_5km.nc?CRW_SST[(",get_date,"T00:00:00Z):1:(",get_date,"T00:00:00Z)][(60):1:(10)][(-150):1:(-100)]",sep="")
    name= "NOAA_DHW"
    acquire_erddap(urls=sst,name=name,final_name=name)
  }
  ...
  if(("l.blendChl.grd" %in% FileList_missing)==TRUE){
    ...
    if(file.exists(paste(tmpdir,"/erdMBchla.nc",sep=""))==TRUE && file.exists(paste(tmpdir,"/erdVH3chla.nc",sep=""))==TRUE  & !file.exists(paste0(finaldir,"/l.blendChl.grd"))){
      ...
      writeRaster(r,paste(finaldir,"/l.blendChl",sep=""),overwrite=T)
    }
    
    # Scenario 2: new layer for MODIS, no new layer for VIIRS
    if(file.exists(paste(tmpdir,"/erdMBchla.nc",sep=""))==TRUE && file.exists(paste(tmpdir,"/erdVH3chla.nc",sep=""))==FALSE   & !file.exists(paste0(finaldir,"/l.blendChl.grd"))){
...
Get_Env_Data_B_1(machine="local")
