...
modisDir="/Users/heatherwelch/Dropbox/JPSS/modis_8DayBin/Satellite"
viirsDir="/Users/heatherwelch/Dropbox/JPSS/viirs_8DayBin/Satellite"
...
dates_m=list.files(modisDir,full.names = T,recursive = T,pattern = ".grd")%>% grep("2016-10-11",.,invert=T,value=T) %>% gsub("/Users/heatherwelch/Dropbox/JPSS/modis_8DayBin/Satellite/","",.) %>% gsub("/l.blendChl.grd","",.)
names(sat_m)=dates_m
...
dates_v=list.files(viirsDir,full.names = T,recursive = T,pattern = ".grd") %>% gsub("/Users/heatherwelch/Dropbox/JPSS/viirs_8DayBin/Satellite/","",.) %>% gsub("/l.blendChl.grd","",.)
names(sat_v)=dates_v
...
# a=master %>% spread(sensor,chla) %>% mutate(MODIS_VIIRS=MODIS-VIIRS) %>% mutate(MODIS_Blended=MODIS-Blended)%>% mutate(VIIRS_Blended=VIIRS-Blended)
# a=a %>% gather(sensor,chla,-c(date,year,month,Blended,MODIS,VIIRS))
# 
# lineplot=ggplot(a,aes(x=date,y=chla))+geom_line(aes(group=sensor,color=sensor),size=.3)+geom_point(aes(color=sensor),size=.6)+
#   scale_x_date(date_breaks="month",date_labels = "%b",date_minor_breaks = "months")+
...
#2.temporal average ####
modis=list.files(modisDir,full.names = T,recursive = T,pattern = ".grd")%>% grep("2016-10-11",.,invert=T,value=T) %>% stack() %>%mask(.,studyarea) %>% crop(.,extent(studyarea)) %>% calc(.,mean,na.rm=T)
