...
ann_text_2017 <- data.frame(date =as.Date("2017-09-13"),chla = .085,lab = "Text",
                            year = factor(2017,levels = c("2015","2016","2017","2018")))
ann_text_2018 <- data.frame(date = as.Date("2018-10-20"),chla = -.055,lab = "Text",
                            year = factor(2018,levels = c("2015","2016","2017","2018")))
...
lineplot_SP=ggplot(species,aes(x=date))+geom_line(aes(y=chla,group=product,color=product),size=.3)+geom_point(aes(y=chla,group=product,color=product),size=.4)+
  ...
  scale_color_manual("Product",values=c("Leatherback"="#518ab1","Sealion"="black","Swordfish"="coral1","Blueshark - Observer"="#8d4138","Blueshark - Tracking"="grey"))+
  facet_grid(~year, scales="free")+labs(x="Month")+labs(y="MODIS minus VIIRS")+theme(legend.position=c(.17,.25),legend.justification = c(.9,.9))+
  ...
  geom_text(data=ann_text,aes(x=date,y=chla),label="B",size=2.5)+
  ...
  geom_text(data=ann_text_2017,aes(x=date,y=chla),label="Box 1",size=2.5)+
  geom_text(data=ann_text_2018,aes(x=date,y=chla),label="Box 2",size=2.5)
...
lineplot_chla_eco=ggplot(chla_eco,aes(x=date))+geom_line(aes(y=chla,group=product,color=product),size=.3)+geom_point(aes(y=chla,group=product,color=product),size=.4)+
  scale_x_date(date_breaks="month",date_labels = "%b",date_minor_breaks = "months")+geom_hline(yintercept=0)+
  scale_color_manual("Product",values=c("Chl-a"="#1a2977","EcoCast"="#bfb939"))+
  facet_grid(~year, scales="free")+labs(x="Month")+labs(y="MODIS minus VIIRS")+theme(legend.position=c(.1,.2),legend.justification = c(.9,.9))+
  theme(axis.text = element_text(size=6),axis.title = element_text(size=6),legend.text=element_text(size=6),legend.title = element_text(size=6),strip.text.y = element_text(size = 6),strip.text.x = element_text(size = 6), strip.background = element_blank())+
  theme(legend.key.size = unit(.5,'lines'))+ylim(c(-.07,.1))+
  geom_text(data=ann_text,aes(x=date,y=chla),label="A",size=2.5)
...
histtt_SP=ggplot(species,aes(x=chla,group=product))+geom_density(aes(fill=product),alpha=.4)+
  ...
  scale_fill_manual("Product",values=c("Leatherback"="#518ab1","Sealion"="black","Swordfish"="coral1","Blueshark - Observer"="#8d4138","Blueshark - Tracking"="grey"))+
  theme(legend.key.size = unit(.5,'lines'))+xlim(c(-.07,.1))+labs(x="MODIS minus VIIRS")+labs(y="Density")+
  theme(legend.position=c(.7,.4))+annotate(geom = "text",x=-.07,y=130,label="B",size=2.5)+
  theme(axis.text = element_text(size=6),axis.title = element_text(size=6),legend.text=element_text(size=6),legend.title = element_text(size=6),strip.text.y = element_text(size = 6),strip.text.x = element_text(size = 6), strip.background = element_blank())
...
histtt_chla_eco=ggplot(chla_eco,aes(x=chla,group=product))+geom_density(aes(fill=product),alpha=.4)+
  geom_vline(xintercept=0,linetype="dashed")+
  scale_fill_manual("Product",values=c("Chl-a"="#1a2977","EcoCast"="#bfb939"))+
  theme(legend.key.size = unit(.5,'lines'))+xlim(c(-.07,.1))+labs(x="MODIS minus VIIRS")+labs(y="Density")+
  theme(legend.position=c(.7,.4))+annotate(geom = "text",x=-.07,y=40,label="A",size=2.5)+
  theme(axis.text = element_text(size=6),axis.title = element_text(size=6),legend.text=element_text(size=6),legend.title = element_text(size=6),strip.text.y = element_text(size = 6),strip.text.x = element_text(size = 6), strip.background = element_blank())
