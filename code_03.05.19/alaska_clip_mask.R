...
e=extent(-180, -130, 50, 75)
savedir="/Users/heatherwelch/Dropbox/JPSS/global/alaska_rasters_mask/";dir.create(savedir)
...
files=list.files("/Users/heatherwelch/Dropbox/JPSS/global/alaska_rasters_mask",full.names = T)%>% grep("ESACCI-OC-L3S",.,value=T) %>% grep(".grd",.,value=T)
esa=stack(files) %>% raster::calc(.,fun=mean,na.rm=T)

files=list.files("/Users/heatherwelch/Dropbox/JPSS/global/alaska_rasters_mask",full.names = T)%>% grep("GlobColour_Merged_GSM",.,value=T)%>% grep(".grd",.,value=T)
glo_gsm=stack(files)%>% calc(.,fun=mean,na.rm=T)

files=list.files("/Users/heatherwelch/Dropbox/JPSS/global/alaska_rasters_mask",full.names = T)%>% grep("GlobColour_Merged_AVW",.,value=T)%>% grep(".grd",.,value=T)
glo_avw=stack(files)%>% calc(.,fun=mean,na.rm=T)

files=list.files("/Users/heatherwelch/Dropbox/JPSS/global/alaska_rasters_mask",full.names = T)%>% grep("nesdisVHNSQchlaMonthly",.,value=T)%>% grep(".grd",.,value=T)
viirs=stack(files)%>% calc(.,fun=mean,na.rm=T)

files=list.files("/Users/heatherwelch/Dropbox/JPSS/global/alaska_rasters_mask",full.names = T)%>% grep("erdMH1chlamday",.,value=T)
modis=stack(files)%>% calc(.,fun=mean,na.rm=T)

writeRaster(esa,"/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/esa.grd")
writeRaster(glo_gsm,"/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/glo_gsm.grd")
writeRaster(glo_avw,"/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/glo_avw.grd")
writeRaster(viirs,"/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/viirs.grd")
writeRaster(modis,"/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/modis.grd")

# ### plotting ####
# modis=raster("/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/modis.grd") 
# viirs=raster("/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/viirs.grd")
# glo_avw=raster("/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/glo_avw.grd")
# glo_gsm=raster("/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/glo_gsm.grd")
# esa=raster("/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/esa.grd")
# 
...
#   
#   png(paste(outputDir,datatype,"_alaska_masked.png",sep=''),width=18,height=12,units='cm',res=400)
...
writeRaster(master,"/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/mean_all_products.grd")
master_sd=stack(modis,viirs,glo_avw,glo_gsm,esa) %>% calc(.,fun=sd)
writeRaster(master_sd,"/Users/heatherwelch/Dropbox/JPSS/global/alaska_averages_mask/sd_all_products.grd")
...
plot_EP_SD=function(raster,datatype,plus,minus,outputDir){
  pal <- colorRampPalette(c("purple4","blue", "cyan", "yellow", "red"))
  ncolors <- 255
  breaks <- seq(-1.2,1.5,,ncolors+1)
  
  png(paste(outputDir,datatype,"_alaska_masked_SD.png",sep=''),width=18,height=12,units='cm',res=400)
  par(mar=c(3,3,.5,.5),las=1,font=2)
  par(mfrow=c(1,1))
  par(oma=c(0,0,0,2))
  ...
  image.plot(raster, col=pal(ncolors), breaks=breaks, ylab="", xlab="", xlim=c(-180, -130),ylim=c(50, 75),legend.cex=.5)
  maps::map('worldHires',add=TRUE,col="white",fill=TRUE)
  text(-160,65,datatype,adj=c(0,0),cex=1.5,col="black")
  points(mplus,cex=.02,pch = ".",col="grey")
  points(mminus,cex=.02,pch = ".",col="black")
  contour(raster, add=TRUE, col="white",levels=c(.4),labcex = 1,lwd=2)
