...
for(i in 1:length(files)){
  ...
  mean=log(raster(name,var="CHL1_mean")+0.001) %>% cellStats(.,stat="mean",na.rm=T)
  df$year[i]=year
  df$month[i]=month
  df$date[i]=date
  df$sensor[i]=sensor
...
dfGlob=df
write.csv(dfGlob,"/Users/heatherwelch/Dropbox/JPSS/global/csvs/Glob_4km_merged_GSM_AVW_log.csv")
...
for(i in 1:length(files)){
  ...
  mean=log(raster(name,var="chlor_a")+0.001) %>% cellStats(.,stat="mean",na.rm=T)
  df$year[i]=year
  df$month[i]=month
  df$date[i]=date
  df$sensor[i]=sensor
...
df$sensor=as.factor(df$sensor)
dfESA=df
write.csv(dfESA,"/Users/heatherwelch/Dropbox/JPSS/global/csvs/ESA_4km_merged_log.csv")
...
for(i in 1:length(files)){
  ...
  if(grepl("erdMH1chlamday", name)){
    ...
    mean=log(raster(name,var="chlorophyll")+0.001) %>% cellStats(.,stat="mean",na.rm=T)
    }
...
df$sensor=as.factor(df$sensor)
dferddap=df
write.csv(dferddap,"/Users/heatherwelch/Dropbox/JPSS/global/csvs/ERDDAP_MODIS_VIIRS_4km_log.csv")
...
a=ggplot(df,aes(x=date,y=mean)) +geom_line(aes(group=sensor,color=sensor,linetype=sensor),size=.5)+geom_point(aes(color=sensor))+
  ...
  scale_color_manual("Product",values=c("VIIRS"="#d3ad06","MODIS"="#0066cc","ESA Merged"="red","GlobColour Merged GSM"="black","GlobColour Merged AVW"="darkgreen"))+ylab("log Chl-a (mg/m3)")+xlab("Year")+
