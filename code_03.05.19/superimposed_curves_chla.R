...
f=f+ylab("Scaled Chl-a influence and density distribution")+xlab("Chla (mg/m3)")+theme(legend.position=c(.1,.9),legend.justification = c(.9,.9))+
  geom_text(aes(x=-4,y=1),label="A",size=2.5)
f
...
f=f+geom_vline(xintercept=2,linetype="dashed")+geom_vline(xintercept=0,linetype="dashed")
f=f+ylab("Scaled Chl-a influence and density distribution")+xlab("Chla (mg/m3)")+theme(legend.position=c(.1,.9),legend.justification = c(.9,.9))+
  geom_text(aes(x=-3,y=1),label="B",size=2.5)
f
