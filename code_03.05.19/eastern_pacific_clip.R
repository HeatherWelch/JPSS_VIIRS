...
savedir="/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_rasters/"
rasterdir="/Users/heatherwelch/Dropbox/JPSS/global/global_rasters"

for(file in list.files(rasterdir,full.names = T,pattern = ".grd")){
  print(file)
  name=gsub("/Users/heatherwelch/Dropbox/JPSS/global/global_rasters/","",file)
  grid=raster(file)
  clip=crop(grid,e)
...
files=list.files("/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_rasters/",full.names = T)%>% grep("ESACCI-OC-L3S",.,value=T) %>% grep(".grd",.,value=T)
esa=stack(files) %>% calc(.,fun=mean,na.rm=T)

files=list.files("/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_rasters/",full.names = T)%>% grep("GlobColour_Merged_GSM",.,value=T)%>% grep(".grd",.,value=T)
glo_gsm=stack(files)%>% calc(.,fun=mean,na.rm=T)

files=list.files("/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_rasters/",full.names = T)%>% grep("GlobColour_Merged_AVW",.,value=T)%>% grep(".grd",.,value=T)
glo_avw=stack(files)%>% calc(.,fun=mean,na.rm=T)

files=list.files("/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_rasters/",full.names = T)%>% grep("nesdisVHNSQchlaMonthly",.,value=T)%>% grep(".grd",.,value=T)
viirs=stack(files)%>% calc(.,fun=mean,na.rm=T)

files=list.files("/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_rasters/",full.names = T)%>% grep("erdMH1chlamday",.,value=T)
modis=stack(files)%>% calc(.,fun=mean,na.rm=T)

writeRaster(esa,"/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_averages/esa.grd")
writeRaster(glo_gsm,"/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_averages/glo_gsm.grd")
writeRaster(glo_avw,"/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_averages/glo_avw.grd")
writeRaster(viirs,"/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_averages/viirs.grd")
writeRaster(modis,"/Users/heatherwelch/Dropbox/JPSS/global/eastern_pacific_averages/modis.grd")
...
ncolors <- 255
breaks <- seq(0.007,1,,ncolors+1)

outputDir="/Users/heatherwelch/Dropbox/JPSS/plots_03.05.19/"
datatype="eastern_pacific_map2"
